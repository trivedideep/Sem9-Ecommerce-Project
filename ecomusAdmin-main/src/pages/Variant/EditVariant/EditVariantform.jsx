...
import { useGetSingleVariantQuery, usePatchVariantMutation } from "../../../store/api/variantapi";
const EditVariantform = ({ id }) => {
  ...
  const [patchvariant] = usePatchVariantMutation();

  const ProductForm = async (value) => {
    try {
      const response = await patchvariant({ data: value, id: id });
      if (!response.error) {
        if (response.data.status === "successfully") {
          // nvg("/productlist/2");
          nvg(`/editproduct/${response.data.data.product_id}`);
  ...
  const { data: data, isLoading } = useGetSingleVariantQuery(id);
  // fetch all product api start here


  // fetch Attribute api start here
  const { data: attributedata,refetch } = useGetAttributeByCategoryIdQuery(
    !subcategoryid[0] ? (!categoryid[0] ? 0 : categoryid[0]) : subcategoryid[0]
  );
  // fetch Attribute api end here


  console.log("fetch attributes of data",attributedata ,categoryid,subcategoryid)
  useEffect(() => {
    if (data) {
      // FetchsubCategorys(data.data.parent_category);
      selectedvalues(data.data.dynamicAttributes);
      if(data.parentcategory[0]){
      setcategoryid([data.parentcategory[0]._id])
      }
      if(data.childcategory[0]){
        setsubcategoryid([data.childcategory[0]._id])
      }
  console.log("fetch attributes of data 32222222",attributedata ,data.parentcategory,data.childcategory)
