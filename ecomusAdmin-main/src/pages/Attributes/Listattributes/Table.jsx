...
import { useDeleteAttributeMutation, useGetAllAttributeQuery } from "../../../store/api/attributeapi";
...
const Table = () => {
  ...
  // delete attribute record start here
  const [deletereord] = useDeleteAttributeMutation();
  const deletecategory = () => {
    const response = deletereord(isdelete);
    setisdelete(0);
    window.location.reload();
  };
  // delete attribute record end here
  ...
  useEffect(() => {
    if (attributeData) {
      async function fetchData() {
        setloading(true);
        try {
          const dataWithSerialNumbers = attributeData.data.map((row, index) => ({
            ...row,
            serialNo: index + 1,
            id: index + 1,
          }));
