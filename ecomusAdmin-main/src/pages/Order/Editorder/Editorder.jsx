...
const Editorder = () => {
  ...
  const { data: orderdetail, isLoading: orderloading, refetch } = useGetSingleOrderQuery(id);
  const [postorderstatus] = usePostOrderStatusMutation();

  const changestatus = async (value, id) => {
    if (!value) {
      setErrorMessage('Please select an order status before saving.');
      return;
    }

    if (orderdetail?.data?.order_status === 'Delivered') {
      setErrorMessage('Delivered orders cannot be updated.');
      return;
    }

    if (orderdetail?.data?.order_status === value) {
      setErrorMessage('Order status is already set to this value.');
      return;
    }

    const data = { order_status: value };
    const response = await postorderstatus({ data, id });
    const statusText = response?.data?.status?.toLowerCase?.() || '';
    if (!response.error && statusText.includes('success')) {
  ...
  useEffect(() => {
    if (orderdetail?.data?.order_status) {
      setstatus(orderdetail.data.order_status);
    }
  }, [orderdetail]);
        ...
        <section className="invoice">
          ...
          <div className="row removeprintbtn" style={{ padding: '10px 22px', display: 'flex', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', width: '60%', padding: '0px' }} className='hidebox1'>
              <select
                className="custom-select hidebox1"
                value={status}
                onChange={(e) => {
              ...
              <button
                className="save-button hidebox1"
                type='button'
                onClick={() => { changestatus(status, orderdetail.data._id) }}
                style={{ background: '#059fe2' }}
                disabled={isDelivered}
              >
                Save
              </button>
            ...
            <div className="col-12" style={{ display: errorMessage === "" ? "none" : "block" }}>
