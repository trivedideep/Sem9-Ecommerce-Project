...
const createorder = async (req, res) => {
    try {
        ...
        if (countcount > 0) {
            ...
            const isPaymentSuccessful = payment_status === 'received' || payment_status === 'success';

            if (isPaymentSuccessful) {
                for (const item of cartItemsForOrder) {
                    const productIdToUpdate = item.product_id; // Using product_id for stock update
                    const quantityToRemove = item.product_qty;

                    // Find and update the product stock
                    await product.findByIdAndUpdate(
                        productIdToUpdate,
                        { $inc: { stock: -quantityToRemove } }, // Decrease stock by quantityToRemove
                        { new: true }
                    );
                }
            ...
            // ... rest of your original logic for email and response ...
        ...
        <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0" style="font-family: Arial, Helvetica, sans-serif; font-size: 12px; border: 1px solid #eee;">
            <tbody>
                ...
                `<tr>
                                    ...
                                    <td style={{ borderBottom: '1px solid black', borderRight: '1px solid black', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '12px' }} width="12%" align="center">&nbsp;${rescart.product_id == null ? (rescart.product_variant_id?.selling_price ?? 0) : (rescart.product_id?.selling_price ?? 0)}</td>
                                    <td style={{ borderBottom: '1px solid black', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '12px' }} width="12%" align="center">&nbsp;${((rescart.product_id == null ? (rescart.product_variant_id?.selling_price ?? 0) : (rescart.product_id?.selling_price ?? 0)) * (rescart.product_qty ?? 0))}</td>
                                </tr>`
            ))}
                                <tr>
...
`;
            ...
            // Ensure we have a recipient: prefer shipping_email.
            // Token only contains user id, so fetch user's email from DB when needed.
            let userEmailFromDb = null;
            if (!shipping_email && req.user && req.user.id) {
                try {
                    const userRec = await usertable.findById(req.user.id).select('email');
                    if (userRec && userRec.email) userEmailFromDb = userRec.email;
                } catch (e) {
                    // don't block order flow if lookup fails; fallback below will cover it
                    console.warn('Could not fetch user email for order email fallback:', e && e.message);
                }
            }
            const recipientEmail = shipping_email || (req.user && req.user.email) || userEmailFromDb || (websiteinfo && websiteinfo[0] && websiteinfo[0].email);
            // Send invoice email only if payment_status is 'received' or 'success'
            if (isPaymentSuccessful) { // Reusing isPaymentSuccessful for consistency
                await sendEmail(recipientEmail, "Order Confirmation", "Order Details With Invoice", emailHtml);
            }
            res.send({ status: "successfully", order: savedOrder, updatedCart });
        } else {
            // Handle case where cart is empty or no items match the status
            res.send({ status: "failed", errors: "No items in cart to create an order." });
        }
    } catch (err) {
